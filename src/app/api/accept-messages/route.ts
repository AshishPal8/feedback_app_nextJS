...
export async function POST(request:Request){
    ...
    try {
        ...
        return Response.json({
            success: true,
            message: "message accepetence status updated successfully",
            updatedUser
        }, {status: 200}) 
...
export async function GET(request: Request){
    ...
    try {
        ...
        console.log("Failed to getting message accepetance status");
        return Response.json({
            success: false,
            message: "Failed to getting message accepetance status"
        }, {status: 500}) 
    }
}